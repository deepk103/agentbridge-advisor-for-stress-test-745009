...
resource "azurerm_resource_group" "main" {
  name     = "rg-agentbridge-${var.environment}-advisor-for-stress-test"
  location = var.region
  tags = {
    Environment = var.environment
    ManagedBy   = "terraform"
    Blueprint   = "Advisor for Stress Test"
  }
}

# Virtual Network
resource "azurerm_virtual_network" "main" {
  name                = "vnet-agentbridge-advisor-for-stress-test"
...
resource "azurerm_subnet" "main" {
  name                 = "snet-agentbridge-advisor-for-stress-test"
  resource_group_name  = azurerm_resource_group.main.name
...
resource "azurerm_network_security_group" "main" {
  name                = "nsg-agentbridge-advisor-for-stress-test"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_public_ip" "main" {
  ...
  name                = "pip-agentbridge-advisor-for-stress-test"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_network_interface" "main" {
  name                = "nic-agentbridge-advisor-for-stress-test"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_linux_virtual_machine" "main" {
  name                  = "vm-agentbridge-advisor-for-stress-test"
  resource_group_name   = azurerm_resource_group.main.name
  location              = azurerm_resource_group.main.location
...
resource "azurerm_managed_disk" "data" {
  ...
  name                 = "disk-agentbridge-advisor-for-stress-test-data"
  location             = azurerm_resource_group.main.location
  resource_group_name  = azurerm_resource_group.main.name
  storage_account_type = "Premium_LRS"
  create_option        = "Empty"
...
resource "azurerm_cognitive_account" "ai_services" {
  ...
  name                  = "agentbridge-advisor-for-stress-test-ai"
  location              = azurerm_resource_group.main.location
  resource_group_name   = azurerm_resource_group.main.name
  kind                  = "AIServices"
  sku_name              = "S0"
  custom_subdomain_name = "agentbridge-advisor-for-stress-test-ai"
...
resource "azurerm_user_assigned_identity" "vm_foundry" {
  count               = var.enable_foundry ? 1 : 0
  name                = "agentbridge-advisor-for-stress-test-vm-identity"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
}
...
resource "null_resource" "foundry_agents" {
  ...
  provisioner "local-exec" {
    interpreter = ["bash", "-c"]
    command     = <<-EOT
      ENDPOINT="https://agentbridge-advisor-for-stress-test-ai.services.ai.azure.com"
